...
async function main() {
    ...
    await remoteHubUpgrader.setRemoteHub(remoteHub.target);
    await exchange.setRemoteHub(remoteHub.target);
    await market.setRemoteHub(remoteHub.target);
    await portfolioManager.setRemoteHub(remoteHub.target);
    await xusdToken.setRemoteHub(remoteHub.target);
    await wrappedXusdToken.setRemoteHub(remoteHub.target);
    await payoutManager.setRemoteHub(remoteHub.target);
