...
async function main() {
    ...
    await deployImpl("ExchangeMother", networkName, [remoteHub.target], [], []);
    await deployImpl("Market", networkName, [remoteHub.target], [], []);
    await deployImpl("RoleManager", networkName, [], [], []);
    await deployImpl("PortfolioManager", networkName, [], [], []);

    await deployImpl("XusdToken", networkName, ["xUSD", "xUSD", 6, remoteHub.target], [], []);
    await deployImpl("WrappedXusdToken", networkName, ["Wrapped xUSD", "wxUSD", 6, remoteHub.target], [], []);
    await deployImpl("ArbitrumPayoutManager", networkName, [remoteHub.target, rewardWallet], [], []);
        
}

async function deployImpl(contractName: string, networkName: string, initParams: any, contrParams: any, unsafeAllow: any) {
