...
async function main() {
    ...
    let portfolioManager = await getContract("PortfolioManager", networkName);
    ...
    await exchange.grantRole(Roles.UPGRADER_ROLE, dev5Signer);
    await market.grantRole(Roles.UPGRADER_ROLE, dev5Signer);
    await payoutManager.grantRole(Roles.UPGRADER_ROLE, dev5Signer);
    await portfolioManager.grantRole(Roles.UPGRADER_ROLE, dev5Signer);
    await remoteHub.grantRole(Roles.UPGRADER_ROLE, dev5Signer);
    await remoteHubUpgrader.grantRole(Roles.UPGRADER_ROLE, dev5Signer);
    await roleManager.grantRole(Roles.UPGRADER_ROLE, dev5Signer);
    await wrappedXusdToken.grantRole(Roles.UPGRADER_ROLE, dev5Signer);
    await xusdToken.grantRole(Roles.UPGRADER_ROLE, dev5Signer);
