...
let contracts = {
    "RemoteHub": {
        "name": "RemoteHub",
        "initParams": [config.chainSelector, config.chainSelector],
        "unsafeAllow": ['constructor', 'state-variable-immutable'],
        "contrParams": [config.ccipRouterAddress]
    },
    "RemoteHubUpgrader": {
        "name": "RemoteHubUpgrader",
        "initParams": [config.chainSelector],
        "unsafeAllow": ['constructor', 'state-variable-immutable'],
        "contrParams": [config.ccipRouterAddress, config.remoteHub]
    },
    "ExchangeChild": {
        "name": "ExchangeChild",
        "initParams": [config.remoteHub],
        "unsafeAllow": [],
        "contrParams": []
    },
    "Market": {
    ...
    "XusdToken": {
        "name": "XusdToken",
        "initParams": ["xUSD", "xUSD", 6, config.remoteHub],
        "unsafeAllow": [],
        "contrParams": []
    },  
    "PayoutManager": {
        "name": "ModePayoutManager",
        "initParams": [config.remoteHub, rewardWallet],
        "unsafeAllow": [],
        "contrParams": []
    },
    "WrappedXusdToken": {
        "name": "WrappedXusdToken",
        "initParams": ["Wrapped xUSD", "wxUSD", 6, config.remoteHub],
        "unsafeAllow": [],
        "contrParams": []
    }
};
...
async function main() {
    ...
    let payoutManager = await getContract("ModePayoutManager");
    ...
    console.log("tut 1", wrappedXusdToken.target);
    console.log("tut 2", await remoteHub.wxusd());
    // console.log("tut 3", await remoteHub.removeChainItem("7264351850409363825"));
    // return;
