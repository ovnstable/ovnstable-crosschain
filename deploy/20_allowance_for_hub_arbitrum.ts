...
async function main() {
    ...
    await remoteHub.allowlistDestinationChain(chain.chainSelector, true);
    await remoteHubUpgrader.allowlistDestinationChain(chain.chainSelector, true);
    await remoteHub.allowlistSourceChain(chain.chainSelector, true);
    await remoteHubUpgrader.allowlistSourceChain(chain.chainSelector, true);
    await remoteHub.allowlistSender(remoteHub.target, true);
    await remoteHub.allowlistSender(remoteHubUpgrader.target, true);
    await remoteHubUpgrader.allowlistSender(remoteHub.target, true);
    await remoteHubUpgrader.allowlistSender(remoteHubUpgrader.target, true);
